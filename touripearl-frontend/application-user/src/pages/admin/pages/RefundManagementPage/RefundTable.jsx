...
const RefundTable = () => {
    ...
    const fetchData = async () => {
        try {
            setIsLoading(true);
            const response = await axiosFetch.get('/api/v1/booking/refundRequests/search-by-keyword', {
                params: {
                    page: page,
                    size: 10,
    ...
    return (
        <Dashboard title="Refund Management">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                    <div className="relative flex-1 max-w-md">
                        ...
                        transition-all duration-200"
                            title="Search by refund name, email, or other details"
                        />
                    </div>
                    <select
                        value={statusFilter}
                            ...
                            <TableBody>
                                {data.content.map((refund, index) => (
                                    <TableRow
                                        ...
                                        key={index}
                                    >
                                        <TableCell className="font-medium">{refund?.propertyId}</TableCell>
                                        ...
                                        <TableCell>{new Date(refund.updatedAt).toLocaleString()}</TableCell>
                                        <TableCell>
                                            <div className="flex gap-2">
                                                <Link
                                                    to={`/admin/refund-management/${refund.propertyId}`}
