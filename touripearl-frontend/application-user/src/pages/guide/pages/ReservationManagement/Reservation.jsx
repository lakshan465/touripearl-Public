...
const Reservation = () => {
  ...
  const confirmReservation = () => {
    setLoading(true);
    axiosFetch.put(`/api/reservations/${reservationId}`, { status: 'CONFIRMED' })
      .then(response => {
        setReservation(response.data);
        setLoading(false);
      })
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl dark:bg-gray-900">
      ...
      <div className="flex justify-center gap-4 mt-8">
        {reservation.status === "PENDING" && <button
          onClick={confirmReservation}
          className="px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 flex items-center gap-2 disabled:opacity-10"
          disabled={reservation.status === 'CONFIRMED'}
        >
          <Check className="h-4 w-4" />
          Confirm Reservation
        </button> }
        <button className="px-6 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 flex items-center gap-2"
        onClick={window.print}>
          <FileText className="h-4 w-4" />
          Print Reservation
        </button>
        <Link
            to='/guide/reservations-management'
          >
